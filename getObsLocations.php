<?php
  ...
  if ($_REQUEST['provider'] == 'CBIBS') {
    $provider = 'CBIBS';
    $json = json_decode(file_get_contents('xml/cbibs.json'),TRUE);
    ...
    if ($type == 'spray') {
      ...
      $json = array_merge(
        ...
        ,preg_match('/&provider\[\]=gerg/',$_REQUEST['filterProvider']) ?
          ...
          : array()
        ,preg_match('/&provider\[\]=glos/',$_REQUEST['filterProvider']) ?
          (array)json_decode(file_get_contents(str_replace(' ','%20','http://'.$_SERVER['SERVER_NAME'].substr($_SERVER['PHP_SELF'],0,strrpos($_SERVER['PHP_SELF'],'/')).'/gliders_glos.php?t0='.$_REQUEST['t0'].'&t1='.$_REQUEST['t1'].'&type=slocum')))
          : array()
      );
    }
    else {
